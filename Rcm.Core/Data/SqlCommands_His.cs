namespace Rcm.Core.Data {
    ...
    public partial class SqlCommands_His {
        ...
        EXECUTE sp_executesql @SQL;";
        public const string Sql_Value_Repository_GetStaticValues = @"
        DECLARE @tpDate DATETIME, 
                @tbName NVARCHAR(255),
                @tableCnt INT = 0,
                @SQL NVARCHAR(MAX) = N'';
        ...
        WHILE(DATEDIFF(MM,@tpDate,@End)>=0)
        BEGIN
            SET @tbName = N'[dbo].[HStaticCurve'+CONVERT(VARCHAR(6),@tpDate,112)+ N']';
            IF EXISTS (SELECT 1 FROM sys.objects WHERE object_id = OBJECT_ID(@tbName) AND type in (N'U'))
            BEGIN
                IF(@tableCnt>0)
                BEGIN
                ...
                SET @SQL += N'SELECT * FROM ' + @tbName + N' WHERE [PointID]='+CAST(@Point AS NVARCHAR)+' AND [BeginTime] BETWEEN ''' + CONVERT(NVARCHAR,@Start,120) + N''' AND ''' + CONVERT(NVARCHAR,@End,120) + N'''';
                SET @tableCnt += 1;
            END
            SET @tpDate = DATEADD(MM,1,@tpDate);
        END
        ...
        BEGIN
	        ...
	        SELECT * FROM TResult ORDER BY [BeginTime];'
        END
