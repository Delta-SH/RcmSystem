...
Ext.define('Ext.ux.TreePicker', {
    ...
    */
    ...
    doRawSearch: function (field, text) {
        ...
        if (!Ext.isEmpty(field._filterData)
            && !Ext.isEmpty(field._filterIndex)) {
    ...
    createPicker: function () {
        ...
        var expandfold = Ext.create('Ext.form.field.Checkbox', {
            boxLabel: '展开/折叠',
            listeners: {
                change: function () {
        ...
        var allSelection = Ext.create('Ext.form.field.Checkbox', {
            boxLabel: '全选',
            listeners: {
                change: function () {
                    var checked = this.getValue();
                    ...
                    if (root.hasChildNodes()) {
                        ...
                        picker.checking = false;
                    }
                    me.onCheckChange(root, checked);
                }
            }
        });

        var inverseSelection = Ext.create('Ext.form.field.Checkbox', {
            boxLabel: '反选',
            listeners: {
                change: function () {
                    var checked = this.getValue();
        ...
        var picker = me.picker = Ext.create('Ext.tree.Panel', {
            ...
            bbar: me.fastSelectVisible ? new Ext.toolbar.Toolbar({
                height: 25,
                padding: '0 10',
                items: [expandfold, '->', allSelection, inverseSelection]
            }) : false
        });
    ...
    onCheckChange: function (node, checked) {
        var me = this;
        if (me.picker.checking === true)
            return;

        var nodes = me.picker.getChecked();
        me.setValue(nodes);
